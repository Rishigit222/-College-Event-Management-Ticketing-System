...
void add_ticket_type(Event *e, const char *name, double price) {
    ...
    strncpy(e->ticketTypes[e->numTicketTypes].name, name, sizeof(e->ticketTypes[0].name)-1);
...
bool book_ticket(Event *e, const char *custName, const char *email, int ticketTypeIndex, int *outBookingId, bool *outWaitlisted) {
